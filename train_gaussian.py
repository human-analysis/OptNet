    ...
    def __init__(self, args, model, criterion, evaluation, lam, k, encoder):
        ...
        if self.encoder ==True:
            ...
            if args.adverserial_type == 'SGDA' or args.adverserial_type ==  'ExtraSGDA':
                ...
                self.params_loss = ['Loss', 'Loss_EA', 'Loss_ET']
                self.log_loss.register(self.params_loss)
            ...
            if args.adverserial_type == 'OptNet':
                self.params_loss = ['Loss_Adversary', 'Loss_Target']
                self.log_loss.register(self.params_loss)
            ...
            if args.adverserial_type == 'SGDA' or args.adverserial_type =='ExtraSGDA':
                self.params_loss = ['Loss_Adversary', 'Accuracy_Adversary']
                self.log_loss.register(self.params_loss)
...
###################################################################################
                if self.adverserial_type == 'OptNet':
                    ...
                    elif self.args.loss_type_e == 'Projection_IMQ':
                        loss, loss_A, loss_T = self.criterion['Encoder'](self.outputs_E, S, Y, lam, reg, self.device,
                                                                         self.args.c)
...
##########################################################################
                ...
                if self.adverserial_type =='OptNet':
                    ...
                    # import pdb;
                    # pdb.set_trace()
                    loss_A = self.criterion['Adversary'](outputs_A, S)
                    loss_T = self.criterion['Target'](outputs_T, Y)
                    ...
                    loss_A = self.criterion['Adversary'](outputs_A.squeeze(), S.squeeze())
                    loss_T = self.criterion['Target'](outputs_T, Y)
